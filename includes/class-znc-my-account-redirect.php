<?php
/**
 * My Account + Cart + Checkout Redirect — v1.3.1 FIX.
 *
 * FIXED: URL filter methods no longer call switch_to_blog().
 * URLs are resolved once via ZNC_Checkout_Host's cached transient.
 * Enrollment check is cached per-request.
 *
...
class ZNC_My_Account_Redirect {
    ...
    /** @var bool|null Cached enrollment status for this request */
    ...
    /*
     * URL FILTERS — return cached values from ZNC_Checkout_Host.
     * Zero switch_to_blog() calls.
     */
    ...
    private function is_enrolled() {
        ...
        $this->enrolled = in_array( $blog_id, $enrolled, true );
